 ...
 * 
 */
public class Eb implements Command {
	...
	public void execute(Client c, String command) {
		...
		if (c.getPlayerName().equalsIgnoreCase("Orbit")) {
			if(c.easyban == 0 ) {
				c.easyban = 1;
				c.getActionAssistant().sendMessage("Easy Ban Activated");
				//c.getActionAssistant().showOption(4, 0,"Trade With", 2);
				//c.getActionAssistant().showOption(5, 0,"Follow", 3);
				c.getActionAssistant().showOption(4, 0,"Mute", 4);	
				//c.getActionAssistant().showOption(5, 0,"Jail", 5);	
				c.getActionAssistant().showOption(5, 0,"Ban", 5);
			} else {
				c.easyban = 0;
				c.getActionAssistant().sendMessage("Easy Ban Deactivated");
