...
public class Region {
	...
	private void delClip(int x,int y,int height){
		int regionAbsX = (id >> 8) * 64;
		int regionAbsY = (id & 0xff) * 64;
    ...
    public static void delClipping(int x, int y, int height) {
        int regionX = x >> 3;
        int regionY = y >> 3;
        int regionId = ((regionX / 8) << 8) + (regionY / 8);
        for (Region r : regions) {
            if (r.id() == regionId) {
                r.delClip(x, y, height);
                break;
            }
        }
    }
	...
	public static final int[] ignoredObjects = { 1596,1597,660,661,662,663,664,665,666,667,734,1341,1342,1343,1344,1345,1346,1347,1348,1349,1350,1351,1352,1353,1354,1355,1356,1357,1358,1359,
		2310,2891,4061,4328,4329,4822,5905,6212,7961,9037,9661,12733,12240};
	
 	public static boolean onOurIgnoreList(int objectId) {
		for (int list : ignoredObjects) 
			if (list == objectId) 
				return true;
		return false;
	}
    ...
    public static void addObject(int objectId, int x, int y, int height, int type, int direction) {
		...
		if(onOurIgnoreList(objectId)){
			if(objectId == 734){
			System.out.println("Object Id: "+objectId+" Type: "+type);
			//addClipping(x, y, height, 0x200000);
			}
		} else if (type == 22 && !onOurIgnoreList(objectId)){		
            if (def.hasActions() && def.aBoolean767()) {
                addClipping(x, y, height, 0x200000);
            }
        } else if (type >= 9 && !onOurIgnoreList(objectId)) {
			if(def.aBoolean767())
			{
    ...
    public static void load() {
    	try {
		File f = new File("config/world/map_index");
        ...
        for (int i = 0; i < size; i++) {
            byte[] file1 = getBuffer(new File("config/world/map/" + mapObjectsFileIds[i] + ".gz"));
            byte[] file2 = getBuffer(new File("config/world/map/" + mapGroundFileIds[i] + ".gz"));
	...
	dis.readFully(buffer);
	dis.close();
	byte[] gzipInputBuffer = new byte[999999]; // the fuck?
	int bufferlength = 0;
